...
class RRTPathFinder{
protected:
    std::vector<GridNodePtr> FromStartNodeSet;//从起始点出发的搜索树节点集合
    std::vector<GridNodePtr> FromEndNodeSet;//从终点出发的搜索树节点集合
    std::vector<GridNodePtr> ObstacleSet;//障碍物节点集合
    GridNodePtr Mid_s_Node,Mid_e_Node;
    ...
    GridNodePtr getNearestNode(GridNodePtr node,std::vector<GridNodePtr> nodeSet);//从搜索树节点中找到最近节点
    GridNodePtr getNextNode(GridNodePtr nearnode,GridNodePtr randnode,double stepsize);//确定next节点
    std::vector<GridNodePtr> getNeighbors(GridNodePtr node,std::vector<GridNodePtr> nodeSet,double R);//获取next节点周围半径R内的节点集合
